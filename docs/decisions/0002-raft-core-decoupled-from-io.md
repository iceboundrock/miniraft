...
the log, and validated at every ingress (`raft.ValidateEntries`: real indexes
starting at >= 1, contiguous without wrapping past the maximum `Index`,
non-zero terms) so a malformed entry can never reach a log.
